...

{
  ...
  for ( i = 0; i < local_lnum_F; i++ ) {
    activate_ft( local_lF[i], 0 );
  }
...

{
  ...
  while ( TRUE ) {
	...
	*max = time;
     ...
     */
    stop_ft = local_lnum_F;
    for ( i = start_ft; i < stop_ft; i++ ) {
      activate_ft( local_lF[i], time );
    }
    if ( time == 0 ) {
      for ( i = 0; i < lnum_0P_E; i++ ) {
	if ( gef_conn[l0P_E[i]].in_E ) {
...

{
  ...
  if ( first_call ) {
     ...
     */
    local_lF = ( int * ) calloc( gnum_ft_conn, sizeof( int ) );
...

{
  ...
  for ( i = 0; i < local_lnum_F; i++ ) {
    gft_conn[local_lF[i]].level = INFINITY;
    gft_conn[local_lF[i]].in_F = FALSE;
  }
